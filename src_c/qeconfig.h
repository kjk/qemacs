...
CmdDef basic_commands[] = {
    ...
    CMD1( KEY_SHIFT_RIGHT, KEY_NONE, "forward-char-ext-sel", do_left_right_dont_move_mark, 1)
    CMD1( KEY_META('b'), KEY_CTRL_LEFT, "backward-word", do_word_right, -1 )
    CMD1( KEY_META('f'), KEY_CTRL_RIGHT, "forward-word", do_word_right, 1 )

    CMD1( KEY_NONE, KEY_PAGEUP, "scroll-down", do_scroll_up_down, -2 )
    CMD1( KEY_NONE, KEY_PAGEDOWN, "scroll-up", do_scroll_up_down, 2 )
    ...
    CMD1( KEY_CTRL('x'), KEY_NONE, "kill-region", do_kill_region, 1 )
    CMD1( KEY_CTRL('c'), KEY_NONE, "copy-region", do_kill_region, 0 )
    ...
    CMD_( KEY_CTRL('v'), KEY_NONE, "yank", do_yank, "*")
    ...
    CMD0( KEY_CTRL('z'), KEY_NONE, "undo", do_undo)
...
CmdDef basic_commands[] = {
...
#else
    CMD1( KEY_META('v'), KEY_PAGEUP, "page-up", do_up_down, -2 )
    CMD1( KEY_CTRL('v'), KEY_PAGEDOWN, "page-down", do_up_down, 2 )
#endif
    ...
    CMD0( KEY_CTRL('l'), KEY_NONE, "refresh", do_refresh_complete)
    /* CG: should take a string if no numeric argument given */
    CMD_( KEY_CTRL('g'), KEY_NONE, "goto-line", do_goto_line, "i{Goto line: }")
    CMD_( KEY_NONE, KEY_NONE, "goto-char", do_goto_char, "i{Goto char: }")
    CMD0( KEY_CTRLX(KEY_CTRL('q')), KEY_NONE, "vc-toggle-read-only", 
          do_toggle_read_only)
    CMD0( KEY_META('~'), KEY_NONE, "not-modified", do_not_modified)
