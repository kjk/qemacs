...
typedef struct QESettings {
    ...
    int     window_dy;
    int     window_monitor;
    /* TODO: list of opened files and lots of other stuff */
...
{
    QEmacsState * qs = &qe_state;
    EditBuffer *  old_buf, *new_buf;

    new_buf = eb_new("*yank*", BF_SYSTEM);
    if (!new_buf)
        return NULL;

    if (++qs->yank_current == NB_YANK_BUFFERS)
        qs->yank_current = 0;
    old_buf = qs->yank_buffers[qs->yank_current];
    if (old_buf)
        eb_free(old_buf);
    qs->yank_buffers[qs->yank_current] = new_buf;
    return new_buf;
}
...
int text_display(EditState *s, DisplayState *ds, int offset)
{
    ...
    for (;;) {
        ...
        if (offset >= eb_total_size(s->b)) {
            ...
            if (c < ' ' && c != '\t') {
                /* TODO: hack for windows: supress displaying ^M if a line ending
                   is 0xd 0xa (\r\n, common for DOS\Windows files.
                   We should have better support for different line encodings
                   (currently qemacs pretty much assumes single \n (0xa) is a newline */
                if (c != 13)
    ...
    while (find_file_next(ffs, filename, sizeof(filename)) == 0) {
        ...
        if (0 == stat(filename, &sb))
        {
            if (S_ISDIR(sb.st_mode))
                pstrcat(filename, sizeof(filename), DIR_SEP_STR);
        }
...
static void (*minibuffer_cb)(void *opaque, char *buf) = NULL;
static void *minibuffer_opaque = NULL;
static EditState *minibuffer_saved_active = NULL;

static EditState *completion_popup_window = NULL;
static CompletionFunc completion_function = NULL;

static StringArray *minibuffer_history = NULL;
static int minibuffer_history_index = 0;
static int minibuffer_history_saved_offset = 0;

ModeDef minibuffer_mode = {0};
...
ModeDef text_mode = {
    ...
    NULL, /* display */
    text_display,
    text_backward_offset, 

    text_move_up_down,
    text_move_left_right_visual,
    text_move_bol,
    text_move_eol,
    text_move_word_left_right,
    text_scroll_up_down,
    NULL, /* scroll_line_up_down */
    text_write_char,
    text_mouse_goto,
    0, /* mode_flags */
    NULL, /* data_type */
...
/* logging into a special *log* buffer. Mostly used for developement */

static EditBuffer *log_buffer = NULL;
static int accept_log_pri = NO_LOGGING;
#define LOG_BUF_NAME "*log*"

/* Set loggin level to 'level'. 'NO_LOGGING' disables logging.
   Otherwise log_level will log only if its 'level' is smaller than
   log_level_pri. Think about it as: 'level' in log_level is a priority, the
   lower the number the higher the priority. */
void set_logging_pri(int log_pri)
{
    accept_log_pri = log_pri;
}

/* log 'txt' to the LOG_BUF_NAME buffer but only if 'level' is lower or equal
   to log_level_pri */
void log_with_pri(int log_pri, char *txt)
{
    if (NO_LOGGING == log_pri)
        return;

    if (log_pri > accept_log_pri)
        return;

    if (!log_buffer)
        log_buffer = eb_new(LOG_BUF_NAME, 0);

    if (!log_buffer)
        return;

    eb_append(log_buffer, (const void*)txt, strlen(txt));
}
...
static CmdOptionDef *first_cmd_options = NULL;
...
static int get_config_filename(char *buf, int buf_len)
{
...
#define WIN_DX "window-dx"
#define WIN_DY "window-dy"
#define WIN_MON "window-monitor"

#if 0
#define IOBUF_SIZE 32768

typedef struct MemBlockList {
    int             len;
    unsigned char * buf;
    struct MemBlockList *  next;
} MemBlockList;

static MemBlockList *mem_block_new(unsigned char *buf, int len)
{
    MemBlockList *  block = (MemBlockList*)malloc(sizeof(MemBlockList));
    if (!block)
        return NULL;

    block->len = len;
    block->next = NULL;
    block->buf = (unsigned char*)malloc(len);
    if (!block->buf)
    {
        free((void*)block);
        return NULL;
...
static void mem_block_free(MemBlockList *block)
{
    free((void*)block->buf);
    free((void*)block);
}

static void mem_block_list_free(MemBlockList *list)
{
    MemBlockList *cur = list, *next;
...
static unsigned char *mem_blocks_to_one(MemBlockList *list)
{
    int             total_size = 0;
    unsigned char * block, *cur_buf;
    MemBlockList *  cur_el = list;

    while (cur_el)
    {
        total_size += cur_el->len;
        cur_el = cur_el->next;
    }
    if (!total_size)
        return NULL;

    block = (unsigned char*)malloc(total_size+1);
    if (!block)
        return NULL;
    block[total_size] = 0;
    cur_buf = block;
    ...
    while (cur_el)
    {
        memmove(cur_buf, cur_el->buf, cur_el->len);
        cur_buf += cur_el->len;
        cur_el = cur_el->next;
    }
    return block;
...
static char *slurp_file(const char *filename)
{
    ...
    FILE *        fp;
    MemBlockList *first_block = NULL, *last_block = NULL;
    MemBlockList *block;
    unsigned char *data;
    ...
    for (;;) {
        len = fread(buf, 1, IOBUF_SIZE, fp);
        if (len < 0)
            goto failed;
        if (len == 0)
            break;
        block = mem_block_new(buf, len);
        if (!last_block)
            first_block = block;
        else
    ...
    data = mem_blocks_to_one(first_block);
    return (char*)data;
failed:
    mem_block_list_free(first_block);
    return NULL;
}
#endif
...
{
    char            file[MAX_PATH];
    json_object *   top_level;
    int             ok;
    int             deleted;

    if (!get_config_filename(file, MAX_PATH))
        return;

    top_level = json_new_object();
    if (!top_level)
        return;

    ok = json_object_add(top_level, WIN_DX, json_new_int(global_screen.width));
    if (ok)
        ok = json_object_add(top_level, WIN_DY, json_new_int(global_screen.height));

    if (ok)
        json_to_file(file, top_level);
    deleted = json_unref(top_level);
    assert(deleted);
}
...
static void settings_load(void)
{
    ...
    settings.window_dx = NOT_SET;
    settings.window_dy = NOT_SET;
    settings.window_monitor = NOT_SET;
    settings.window_x = NOT_SET;
    settings.window_y = NOT_SET;

    if (!get_config_filename(file, MAX_PATH))
        return;

    top_level = json_from_file(file);
    if (!top_level)
        return;

    tmp = json_object_get(top_level, WIN_DX);
    if (tmp)
        settings.window_dx = json_get_int(tmp);
    tmp = json_object_get(top_level, WIN_DY);
    if (tmp)
        settings.window_dy = json_get_int(tmp);
