        ...
        *q = '\0';
        ...
        copy:
            ...
            if (buf[0] != '\0' || (buf[0] == '\0' && abs_path))
                pstrcat(buf, buf_size, DIR_SEP_STR);
...
static int is_unix_home(const char *path1)
{
    /* on windows always false */
#ifndef WIN32
    if (*path1 == '~' && (path1[1] == '\0' || path1[1] == DIR_SEP_CHAR))
        return 1;
#endif
    return 0;
}

/* expand unix-style home path (~) in 'path_src' to full path in 'path_dst'.
   return TRUE if expansion took place, FALSE if 'path_src' isn't home path */
static int expand_unix_home(const char *path_src, char *path_dst, int path_dst_len)
{
    char *homedir;
    /* XXX: should call it again */
    if (!is_unix_home(path_src))
        return 0;
    ...
    pstrcpy(path_dst, path_dst_len, homedir);
    pstrcat(path_dst, path_dst_len, path_src + 1);
    return 1;
}
...
{
    const char *p;
    p = strrchr(filename, DIR_SEP_CHAR);
    if (!p) {
...
void skip_non_spaces(const char **pp)
{
    const char *p;

    p = *pp;
    while ((0 != *p) && !css_is_space(*p))
        p++;
    *pp = p;
}
