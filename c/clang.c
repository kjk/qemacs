...
static void do_c_indent(EditState *s)
{
    ...
    /* we only indent if everything before our character is white-space only */
    if (!eb_is_empty_from_to(s->b, offset, offset1))
        return;
...
void do_c_electric(EditState *s, int key)
{
    ...
    /* TODO: this is a hack. If we end up with the cursor position at the
       character we typed (instead of after it), we need to advance the
       cursor position. do_c_indent should probably do the right thing instead */
...
static int c_mode_probe(ModeProbeData *p)
{
    ...
    if (*r) {
        if (strfind("|c|e|h|hpp|hxx|js|cs|jav|java|cxx|cpp|", r + 1, 1))
            return 100;
    }
    return 0;
}
